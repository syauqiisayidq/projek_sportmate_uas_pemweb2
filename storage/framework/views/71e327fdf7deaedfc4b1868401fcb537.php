...
<?php $__env->startSection('content'); ?>
    ...
    <div class="grid grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-5 border border-slate-100">
            <p class="text-2xl font-extrabold text-navy"><?php echo e($totalUsers); ?></p>
            <p class="text-xs text-slate-400">Total Users</p>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-slate-100">
            <p class="text-2xl font-extrabold text-navy"><?php echo e($totalEvents); ?></p>
            <p class="text-xs text-slate-400">Total Events</p>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-slate-100">
            <p class="text-2xl font-extrabold text-navy"><?php echo e($totalFriendRequests); ?></p>
            <p class="text-xs text-slate-400">Friend Requests</p>
        </div>
        <div class="bg-white rounded-2xl p-5 border border-slate-100">
            <p class="text-2xl font-extrabold text-navy"><?php echo e($activeUsers); ?></p>
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="col-span-2 bg-white rounded-2xl p-5 border border-slate-100">
            ...
            <div class="flex items-end gap-3 h-40">
                <?php $max = max(1, $aktivitasMingguan->max('total')); ?>
                <?php $__currentLoopData = $aktivitasMingguan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex-1 flex flex-col items-center gap-2">
                        <div class="w-full bg-primary/20 rounded-t-lg relative" style="height: <?php echo e(max(6, ($d['total']/$max)*100)); ?>%">
                            <div class="absolute inset-x-0 bottom-0 bg-primary rounded-t-lg" style="height:100%"></div>
                        </div>
                        <span class="text-xs text-slate-400"><?php echo e($d['label']); ?></span>
                    </div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sportmate_pemwebII-main\sportmate\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>