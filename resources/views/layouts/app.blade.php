...
<head>
    ...
    <title>@yield('title', 'SportMate')</title>
...
<div class="flex min-h-screen">
    ...
    <div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-1">
            ...
            <a href="{{ route('events.create') }}" class="flex items-center justify-center gap-2 mt-4 bg-primary text-white rounded-xl py-2.5 font-semibold text-sm shadow-sm hover:bg-primary-dark transition">
            ...
            @auth
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 mt-2 rounded-xl text-sm font-medium text-navy bg-slate-50 hover:bg-slate-100">
    ...
    <div class="md:hidden fixed top-0 inset-x-0 bg-white border-b border-slate-100 flex items-center justify-between px-4 py-3 z-30">
        ...
        <a href="{{ route('profile.show') }}">
            <img src="{{ auth()->user()->foto_url }}" class="w-8 h-8 rounded-full object-cover">
        </a>
    </div>
...
<div class="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-slate-100 flex items-center justify-around py-2 z-30">
    <a href="{{ route('dashboard') }}" class="p-2 {{ request()->routeIs('dashboard') ? 'text-primary' : 'text-slate-400' }}">
    ...
    <a href="{{ route('explore.index') }}" class="p-2 {{ request()->routeIs('explore.*') ? 'text-primary' : 'text-slate-400' }}">
    ...
    <a href="{{ route('events.create') }}" class="p-3 bg-primary rounded-full text-white -mt-6 shadow-lg">
        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M11 5h2v6h6v2h-6v6h-2v-6H5v-2h6z"/></svg>
    </a>
    <a href="{{ route('events.index') }}" class="p-2 {{ request()->routeIs('events.*') ? 'text-primary' : 'text-slate-400' }}">
    ...
    <a href="{{ route('profile.show') }}" class="p-2 {{ request()->routeIs('profile.*') ? 'text-primary' : 'text-slate-400' }}">
